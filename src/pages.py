...
def stepwise_training_page():
    ...
    # Randomize the order of tasks 1 and 2
    task_order = random.sample([1, 2], 2)

    # Initialize session state for task tracking
    if 'current_task' not in st.session_state:
        st.session_state.current_task = 0
        st.session_state.task_order = task_order
        st.session_state.adversarial_order = random.choice(["lie_to_truth", "truth_to_lie"])
    ...
    if st.session_state.current_task == 0:
        ...
        st.session_state.page = 'task_2'
    elif st.session_state.current_task == 2:
        st.session_state.page = 'task_3'
    elif st.session_state.current_task == 3:
        st.session_state.page = 'task_4'
    st.experimental_rerun()
...
def load_statements():
    """Load the hippocorpus_test_set dataset."""
